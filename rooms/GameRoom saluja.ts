...
export class State extends Schema {
    ...
    createPlayer(varifyId: string, id: string, seat: number,user_profile:number,user_name:string) {
		...
		// console.log(dtx);
	    // console.log("**<<**");
            dto = { varifyId: varifyId, colyseus_id: id, last_updated_user_coin:dtx.usercoins ,last_updated_win_coin:dtx.wincoins  };
	// console.log("****");
        // console.log(dto);
        // console.log("****");
...
export class SocketRoom extends Room<State> {
    ...
    onCreate(options: any) {
        ...
        console.log(options.maxPlayers);
                    ...
                    for (let key in this.state.players) {
                        ...
                        User.find(id).then((result: any) => {
                            ...
                            RoomCoin.find(id).then((resultx: any) => {
                                ...
                                } else if((dt.wincoins + dt.usercoins) >= dtx.coins) {

                                    console.log({ "error": "testing", "errorvalue": true });
                                    var remaincoin = dtx.coins - dt.usercoins;
                                    var coinsremain = dt.wincoins - remaincoin;
                                    console.log(coinsremain);
                                    var updated_data_id = { _id: new ObjectId(dt._id) };
                                    let updated_data = {
                                        $set: { "wincoins": coinsremain,"usercoins": 0 }
                                    };
                                    // console.log(updated_data);
                                    User.update(updated_data_id, updated_data).then(() => {
        ...
        switch (data.playerPhase) {
            case "roll_dice":
                ...
                if (player.isRolledDice == false) {
                    ...
                    if (player.dicevalue == 6||player.dicevalue == 1) {
                ...
                break;
            case "open_token":
                if (!player.mytokens[data.tokenid].isOpened&&player.dicevalue==1) {
                ...
                }else if (!player.mytokens[data.tokenid].isOpened) {
                    player.mytokens[data.tokenid].isOpened = true;
                    player.mytokens[data.tokenid].pos = 1;
                    player.mytokens[data.tokenid].isGoaled = false;
                    player.opened++;
            ...
            User.find(id).then((result: any) => {
                ...
                RoomCoin.find(idcoin).then((resultx: any) => {
                    ...
                    if (this.maxClients == 2) {
                        ...
                        console.log("***********0***********2***");
                        console.log(dtx.players.twoplayers.totalcoins);

                        console.log(nintypercent);
                        console.log("***********0**********2****");

                        var wonCoins = nintypercent;

                        console.log("***********11**********2****");
                        console.log(dt.wincoins);

                        console.log(wonCoins);
                        console.log("***********11**********2****");
                        var coinsremain = dt.wincoins + wonCoins;
                        ...
                        console.log("***********0**********4****");
                        console.log(dtx.players.fourplayers.totalcoins);
                        ...
                        console.log("***********0***********3***");
                        console.log(dtx.players.threeplayers.totalcoins);
        ...
        if (this.state.time >= this.state.turndelay + 1000) {
            ...
            for (let id in this.state.players) {
                if (this.state.players[id].seat == this.state.turn) {
                    ...
                    if(this.state.players[id].lazymoves>=3){

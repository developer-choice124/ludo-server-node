...
export class Player extends Schema {
    ...
    isNeedLuck(){
            ...
            if (this.previousValues.length == 5&&!this.isUsedLuck) {
                for (let i = 0; i < 5; i++) {
                ...
                if (this.previousValues.length >= 9) {
                    for (let i = 0; i < 9; i++) {
                        if (this.previousValues[i] == 6) {
                            sixes++;
                        }
                    ...
                    if(sixes==0){
                        ...
                        console.log(this.name+"is used his one 10 noluck 6");
                        return true;
...
export class State extends Schema {
    ...
    @type("number")
    readonly turndelay: number = 30000;
...
export class SocketRoom extends Room<State>{ 
    ...
    onCreate(options: any) {
        ...
        this.onMessage("*", (client, message:Message) => {
            console.log(client.sessionId," inputs");
            this.OnMessage(client, message);
        });
        console.log("Room-Created-by! : ", options);
    ...
    onJoin(client: Client, options: any) {
        //checking if same player is joining!
        const samePlayer  = Object.values(this.state.players).find((p:Player) =>{p.varifyId ==options.id});
        console.log("Checking-same-player...." ,samePlayer?true:false)
        if(samePlayer){
            client.leave();
            return;
        }
        console.log(options.user_name,`${options.id}`," joined-room")
        ...
        if (this.state.players._indexes.size+1 == this.maxClients) {
            ...
            console.log( "Phase-changed : ", this.state.gamePhase)
            ...
            .then((resultx1: any) => {
                ...
                RoomDetails.create(this.state.gameBody)
                .then((result: any) => {
                    let data : any = result;
                                ...
                                if (dt.usercoins >= dtx.coins) {
                                    ...
                                    var updated_data = {
                                        ...
                                        User.update(updated_data_id, updated_data).then(() => {
                                            console.log("Users coins deducted");
                                        }).catch((err: any) => {
                                            console.log("failed to deduct users coins",err);
                                        });
                                } else {
                                    console.log("One of the user doesn't have enough coins to proceed!",this.state.players[key].varifyId);
                                }
                            })
                            .catch((err: any) => {
                                console.log("User not found with id ",this.state.players[key].varifyId);
                            });
                        }).catch((err: any) => {
                ...
                .catch((error: any) => {
                    console.log("Failed inserting room details!",error);
                });
            })
            .catch((err: any) => {
                console.log("Failed fetching provided room id ",err);
            });
        ...
        if(this.state.gamePhase=="waiting"){
            ...
            if (consented) {
                ...
                try {
                    ...
                    console.log("Reconnected Client" + client.id);
    ...
    onMessageHandle(id:string,data:Message){
        ...
        switch (data.playerPhase) {
            ...
            case "finished_turn":
                ...
                player.Islazy = false;
                player.playerPhase = data.playerPhase;
                this.broadcast("turn",this.state.turn);
            }
        ...
        if (_id) {
            ...
            User.find(id).then((result: any) => {
                ...
                RoomCoin.find(idcoin).then((resultx: any) => {
                    ...
                    let coinsremain;
                    if (this.maxClients == 2) {
                        let nintypercent = (dtx.players.twoplayers.totalcoins * 95) / 100;
                        let wonCoins:any = nintypercent;
                        coinsremain = dt.usercoins + wonCoins;
                    } else if (this.maxClients == 4) {
                        // console.log(dtx.players.fourplayers);
                        let nintypercent = (dtx.players.fourplayers.totalcoins * 95) / 100;
                        ...
                        // console.log(dtx.players.fourplayers);
                        let nintypercent =  (dtx.players.threeplayers.totalcoins * 95) / 100;
                        // console.log(seventyfivepercent)
                        let wonCoins:any = nintypercent;
                        // console.log(wonCoins);
                        coinsremain = dt.usercoins + wonCoins;
                    ...
                    User.update(updated_data_id_p, updated_data_p).then((resx: any) => {
                        ...
                        if (res.result.nModified == 1) {
                            ...
                            RoomDetails.update(updated_data_id_x, updated_data_x).then((resxxp: any) => {
                                var resk: any = resxxp;
                                if (resk.result.nModified == 1) {
            ...
            for (let id in this.state.players) {
                if (this.state.players[id].seat == this.state.turn) {
                    ...
                    if (!this.state.players[id].Islazy && !this.state.players[id].isMovedToken) {
                        ...
                        if(!this.state.players[id].isBot){
    ...
    movetoken(player:Player,to:number,tid:any){
        ...
        let t = (player.dicevalue / 3) * 1000;
        setTimeout(() =>{
        player.mytokens[tid].pos = to;
        },t);
    ...
    checkfinish(player:Player){
        ...
        if (this.moveabletknscount == 0 && this.openabletknscount == 0) {
            ...
            setTimeout(() => {
                this.finishturn(player.sessionId);
                console.log(" 1028 auto  finish ",player.name ," checkfinish");
            }, 400);
    ...
    CollectPlayerData(player:Player,value:any){
        ...
        for(let key in player.mytokens){
            ...
            if(t.isOpened&&!t.isGoaled&&(t.pos+value)<=57){
        ...
        console.log(this.moveabletknscount , " are moveable of ",player.name );
    }
}

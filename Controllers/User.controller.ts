...
class UserController {
	...
	this.userCoinsUpdate = this.userCoinsUpdate.bind(this);
    }

    async userCoinsUpdate(req: Request, res: Response, next: NextFunction) {
        let body = req.body;
            ...
            await User.jwtAuth(jwt[1], Config.publicKEY, Config.verifyOptions).then(result => {
                ...
                if (data.superSecret == Config.superSecret) {
                    ...
                    function broughtCoin(criteria:any,skip:number){
                        User.findAllcriteria(criteria,skip).then((data:any) => {
                            if (data.length>=100) {
                                skipped = skip + 100;
